resource "aws_scheduler_schedule" "keep-cms-warm" {
  name       = "${local.stack_prefix}-schedule-to-keep-cms-warm"
  group_name = "default"
  state      = "DISABLED"
...
resource "aws_scheduler_schedule" "keep-angular-warm" {
  name       = "${local.stack_prefix}-schedule-to-keep-angular-warm"
  group_name = "default"
  state      = "DISABLED"
...
resource "aws_iam_role" "scheduler_role" {
  name               = "${local.stack_prefix}-cron-scheduler-role"
...
resource "aws_iam_role_policy" "eventbridge_invoke_policy" {
  name   = "${local.stack_prefix}-EventBridgeInvokeLambdaPolicy"
